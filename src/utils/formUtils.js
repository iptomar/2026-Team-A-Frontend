...
export const obterCategorias = (formularios = []) => {
  ...
  return [...new Set(categorias)].sort((categoriaA, categoriaB) =>
    categoriaA.localeCompare(categoriaB, 'pt', { sensitivity: 'base' })
  );
};

export const filtrarEOrdenarFormularios = (
  formularios = [],
  {
    pesquisa = '',
    categoria = 'Todas',
  ...
  const resultados = formularios.filter((formulario) => {
    const correspondePesquisa =
      !pesquisaNormalizada ||
      normalizarTexto(formulario.titulo).includes(pesquisaNormalizada) ||
      normalizarTexto(formulario.descricao).includes(pesquisaNormalizada);

    const correspondeCategoria =
      categoria === 'Todas' ||
      normalizarCategoria(formulario) === categoria;

    const correspondeEstado =
      estado === 'Todos' ||
      formulario.estado === estado;

    const correspondePendentes =
      !apenasComPendentes ||
      Number(formulario.submissoesPendentes || 0) > 0;

    return (
      correspondePesquisa &&
      correspondeCategoria &&
      correspondeEstado &&
      correspondePendentes
    );
  });
  ...
  return [...resultados].sort((formularioA, formularioB) => {
    ...
    const dataA = new Date(
      formularioA.criadoEm || formularioA.createdAt || 0
    ).getTime();

    const dataB = new Date(
      formularioB.criadoEm || formularioB.createdAt || 0
    ).getTime();
...
export const agruparFormulariosPorCategoria = (formularios = []) => {
  ...
  return Object.fromEntries(
    Object.entries(agrupado).sort(([categoriaA], [categoriaB]) =>
      categoriaA.localeCompare(categoriaB, 'pt', { sensitivity: 'base' })
    )
  );
};
